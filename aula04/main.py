...
while not salario_valido:        
    try:
        salario: float = float(input(f"Agora {nome}, Digite o valor do seu salário: "))
...
while not bonus_valido:
    try:
        bonus: float = float(input(f"Por fim {nome}, Digite o valor do bônus recebido (pode ser em %): "))
    ...
    bonus_recebido: float = constante_bonus + salario * bonus
